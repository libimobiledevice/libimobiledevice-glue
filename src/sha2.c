...
#include "libimobiledevice-glue/sha2.h"
...
/* compress 1024-bits */
static int LIMD_sha512_compress(LIMD_sha512_context *md, unsigned char *buf)
{
...
int LIMD_sha512_update (LIMD_sha512_context * md, const unsigned char *in, size_t inlen)
{
    ...
    while (inlen > 0) {
        if (md->curlen == 0 && inlen >= 128) {
           if ((err = LIMD_sha512_compress (md, (unsigned char *)in)) != 0) {
              return err;
           }
           md->length += 128 * 8;
           in             += 128;
           ...
           inlen          -= n;
           if (md->curlen == 128) {
              if ((err = LIMD_sha512_compress (md, md->buf)) != 0) {
                 return err;
              }
              md->length += 8*128;
              md->curlen = 0;
...
int LIMD_sha512_final(LIMD_sha512_context * md, unsigned char *out)
{
    ...
    if (md->curlen > 112) {
        ...
        LIMD_sha512_compress(md, md->buf);
        md->curlen = 0;
    }
...
{
    LIMD_sha512_context ctx;
    int ret;
    if ((ret = LIMD_sha512_init(&ctx))) return ret;
    if ((ret = LIMD_sha512_update(&ctx, message, message_len))) return ret;
    if ((ret = LIMD_sha512_final(&ctx, out))) return ret;
    return 0;
}
...
{
    return LIMD_sha512_final(md, out);
}

int LIMD_sha384_update(LIMD_sha384_context * md, const unsigned char *in, size_t inlen)
{
    return LIMD_sha512_update(md, in, inlen);
}

int LIMD_sha384(const unsigned char *message, size_t message_len, unsigned char *out)
{
    LIMD_sha384_context ctx;
    int ret;
    if ((ret = LIMD_sha384_init(&ctx))) return ret;
    if ((ret = LIMD_sha384_update(&ctx, message, message_len))) return ret;
    if ((ret = LIMD_sha384_final(&ctx, out))) return ret;
    return 0;
}
